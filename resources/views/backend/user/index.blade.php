...
@section('content')
                    ...
                    @foreach ($users as $row)
                      <tr id="tr_object_id_{{ $row->id }}" class="bgc-h-orange-l4">
                        ...
                        <td>
                          ...
                          {{-- <input id="status" name="status" data-id="{{ $row->id }}" {{ $row->status?'checked':'' }} title="Status" type="checkbox" checked="" class="ace-switch" data-bootstrap-switch="" data-off-color="danger" data-on-color="success"> --}}
                        </td>
                        <td>
          ...
          <div class="card">
            <div class="card-header text-white border-0">
              ...
              <div class="mb-2 mb-sm-0">
                <a id="restoreAllObject" href="{{ route('admin.'.$crudRoutePath.'.restoreAll') }}" class="btn btn-blue px-3 d-block w-100 text-95 radius-round border-2 brc-black-tp10">
  ...
  @include('backend.user.templates.crudModal')
@endsection
    ...
    $(document).ready(function () {
      ...
      $('body').on('click', '#objectEdit', function (e) {
        ...
        $.get( actionUrl +'/' +object_id+'/edit', function (res) {
          ...
          form.find('#password').val(res.data.password);
          form.find('#password_confirmation').val(res.data.password);
          ...
          $('.select').val(selectRoles).trigger('change');
          ...
          if (result.value) {
            $.ajax({
              ...
              success: function (data) {
                ...
                var newRowHtml = `
                    @include('backend.user.templates.ajax_tr_trash')
                `;
              ...
              if (result.value) {
                  ...
                  $.ajax({
                    ...
                    if ($('#objectListTrashed tbody tr').length === 0) {
                        ...
                        var newRowHtml = `
                            @include('backend.user.templates.ajax_tr')
                        `;
